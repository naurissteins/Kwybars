...
pub fn render_fake_bars(canvas: &mut [u8], width: u32, height: u32) {
    clear(canvas);
    ...
    for index in 0..bar_count {
        ...
        let bar_height = fake_bar_height(index, bar_count, drawable_height);
        fill_rect(
            canvas,
            width,
            height,
...
fn fake_bar_height(index: usize, bar_count: usize, drawable_height: u32) -> u32 {
    let phase = if bar_count <= 1 {
        0.0
    } else {
        index as f64 / (bar_count - 1) as f64
    };
    let primary = (phase * TAU * 1.15).sin().abs();
    let secondary = (phase * TAU * 0.55 + 0.8).cos().abs();
    let amplitude = (primary * 0.68) + (secondary * 0.32);
    let normalized = 0.18 + amplitude * 0.72;
...
mod tests {
    ...
    fn fake_bars_leave_transparent_background() {
        ...
        render_fake_bars(&mut canvas, width, height);
    ...
    fn fake_bars_handle_small_buffers() {
        ...
        render_fake_bars(&mut canvas, 8, 8);
