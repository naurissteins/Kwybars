...
pub fn for_each_floating_particle(
    ...
    match orientation {
        BarOrientation::Horizontal => {
            ...
            let max_diameter = max_radius * 2.0;
            if max_diameter * scale > height * 0.5 {
                scale = (height * 0.5 / max_diameter).min(scale);
            }
            ...
            let travel = (height - 2.0 * max_r).max(0.0);
        ...
        BarOrientation::Vertical => {
            ...
            let max_diameter = max_radius * 2.0;
            if max_diameter * scale > width * 0.5 {
                scale = (width * 0.5 / max_diameter).min(scale);
            }
            ...
            let max_r = particle_diameter * 0.5;
            let travel = (width - 2.0 * max_r).max(0.0);
